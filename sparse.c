				   ...
				   struct backed_block *bb)
{
	...
	switch (backed_block_type(bb)) {
		case BACKED_BLOCK_DATA:
			ret = write_data_chunk(out, backed_block_len(bb),
		      ...
		      bool crc)
{
	...
	out = output_file_open_fd(fd, s->block_size, s->len, gz, sparse, chunks,
				  crc);
			 ...
			 void *priv)
{
	...
	chunks = sparse_count_chunks(s);
	out = output_file_open_callback(write, priv, s->block_size, s->len,
			      ...
			      void *priv)
{
	...
	chunks = sparse_count_chunks(s);
	out =
	    output_file_open_callback(foreach_chunk_write, &chk, s->block_size,
				      s->len, false, sparse, chunks, crc);
...
int64_t sparse_file_len(struct sparse_file *s, bool sparse, bool crc)
{
	...
	out =
	    output_file_open_callback(out_counter_write, &count, s->block_size,
				      s->len, false, sparse, chunks, crc);
	if (!out)
		return -1;
						  ...
						  unsigned int len)
{
	...
	out_counter =
	    output_file_open_callback(out_counter_write, &count, to->block_size,
				      to->len, false, true, 0, false);
	if (!out_counter)
		return NULL;
	...
	for (bb = start; bb; bb = backed_block_iter_next(bb)) {
		...
		if (file_len + count > len) {
			/*
			 * If the remaining available size is more than 1/8th of
			 * the requested size, split the chunk.  Results in
			 * sparse files that are at least 7/8ths of the
			 * requested size
			 */
...
move:
	backed_block_list_move(from->backed_block_list, to->backed_block_list,
			       start, last_bb);
