...
uint32_t parse_ascii_hex32(const char *s)
{
	uint32_t x = 0;
...
static int decode_programmer_archive(struct sahara_image *images)
{
	...
	for (;;) {
		if (ptr + sizeof(*hdr) > end) {
			fprintf(stderr, "programmer archive is truncated\n");
			return -1;
		}
		hdr = ptr;

		if (memcmp(hdr->c_magic, "070701", 6)) {
			fprintf(stderr, "expected cpio header in programmer archive\n");
			return -1;
		}
		...
		if (namesize > sizeof(name)) {
			fprintf(stderr, "unexpected filename length in progammer archive\n");
			return -1;
		}
		...
		if (id == 0 || id >= MAPPING_SZ) {
			fprintf(stderr, "invalid image id \"%s\" in programmer archive\n", tok);
			return -11;
		}

		ptr += namesize;
		ptr = (void *)(((uintptr_t)ptr + 3) & ~3);
...
static int decode_sahara_config(struct sahara_image *images)
{
	...
	if (!doc) {
		fprintf(stderr, "failed to parse sahara_config in \"%s\"\n", blob->name);
		return -1;
	}
	...
	if (xmlStrcmp(root->name, (xmlChar *)"sahara_config")) {
		fprintf(stderr, "specified sahara_config \"%s\" is not a Sahara config\n", blob->name);
		goto err_free_doc;
	}
	...
	if (!images_node) {
		fprintf(stderr, "no images definitions found in sahara_config \"%s\"\n", blob->name);
		goto err_free_doc;
	}
	...
	for (image_node = images_node->children; image_node; image_node = image_node->next) {
		...
		if (image_id == 0 || image_id > MAPPING_SZ || errors) {
			fprintf(stderr, "invalid sahara_config image in \"%s\"\n", blob->name);
			goto err_free_doc;
		}
...
static int decode_programmer(char *s, struct sahara_image *images, bool *single)
{
	...
	for (pair = strtok_r(s, ",", &save1); pair; pair = strtok_r(NULL, ",", &save1)) {
		...
		if (!id_str || !filename) {
			fprintf(stderr, "failed to parse programmer specifier\n");
			return -1;
		}

		id = strtoul(id_str, NULL, 0);
		if (id == 0 || id >= MAPPING_SZ) {
			fprintf(stderr, "invalid image id \"%s\"\n", id_str);
			return -11;
		}
