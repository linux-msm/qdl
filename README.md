...
On some targets multiple files needs to be loaded in order to reach the
...
Flattened METAs does include the various images that needs to be loaded to
...
```bash
qdl shara_programmer.xml rawprogram.xml
```

#### Programmer archive

Directly providing a list of ids and filenames is cumbersome and error prone,
QDL therefor accepts a "*programmer archive*". This allows the user to use the
